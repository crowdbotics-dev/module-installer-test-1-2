...
export default function CreateChannel({
  ...
  useEffect(() => {
    setContacts(state.contacts.map(obj => ({ ...obj,
      isSelected: false
    })));
  }, [state.contacts]);
    ...
    return <View key={item.id} style={styles.ListItem}>
        <View style={styles.ProfileContainer}>
          <View style={styles.CheckBoxContainer}>
            ...
            true: "#4CAF50",
            false: "lightgray"
          }} value={item.isSelected} onValueChange={newValue => handleCheckBox(newValue, item)} />
          </View>
  ...
  const createGroup = async () => {
    ...
    setLoading(true);
    const res = await createGroupChannel(pubnub, contacts, state.user._id, {
      name,
      custom: {
  ...
  return <>
      ...
      <View style={styles.Container}>
        ...
        item
      }) => ListItem(item)} keyExtractor={item => item._id} />
        <Button disabled={!!(contacts.filter(obj => {
        return obj.isSelected;
      }).length === 0 || loading)} onPress={createGroup} title="Create Group" />
      </View>
    </>;
}
