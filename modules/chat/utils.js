...
export const sortArray = arr => {
  ...
  return newData.sort((a, b) => {
    ...
    const keyB = new Date(b.createdAt); // Compare the 2 dates
...
export const createGroupChannel = async (pubnub, chatWithContactsIds, userId, customData) => {
  // eslint-disable-next-line no-async-promise-executor
  return new Promise(async (resolve, reject) => {
    const channel = chatWithContactsIds.filter(obj => {
      return obj.isSelected;
    }).map(user => user._id).join("-");
    await pubnub.objects.setChannelMetadata({
      channel,
      data: customData
    });
    await pubnub.objects.setChannelMembers({
      channel,
      uuids: chatWithContactsIds.filter(obj => {
        return obj.isSelected;
      }).map(user => user._id)
    });
    await pubnub.channelGroups.addChannels({
...
export const removePubnubChannel = (pubnub, userId, channelId) => {
  return Promise.all([pubnub.objects.removeChannelMetadata({
    channel: channelId
...
export const leavePubnubChannel = (pubnub, userId, channelId) => {
  return Promise.all([pubnub.objects.removeChannelMembers({
    channel: channelId,
    uuids: [userId]
  }), pubnub.channelGroups.removeChannels({
    channelGroup: userId,
