...
const RemotePushController = async (authToken, userID) => {
  ...
  const ENABLED_STATUSES = [messaging.AuthorizationStatus.AUTHORIZED, messaging.AuthorizationStatus.PROVISIONAL];
  const isEnabled = ENABLED_STATUSES.includes(authStatus); // Checks if required permissions are allowed or not

  if (isEnabled) {
    const registrationToken = await messaging().getToken();
    const androidId = await getAndroidId();
    const iosId = await getUniqueId(); // API which registers the device details and FCM token in backend
